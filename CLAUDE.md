...
- `scripts/` — zero-dependency Node tooling: `build.js` (site generator), `sync-evidence.js`, `validate-ontology.js`, `verify-features.js` (multi-model cascade), `check-links.js` / `check-links-browser.js`, `scan-secrets.js`, `mcp-server.js` (read-only MCP server over generated JSON, config in `mcp.json`), `generate-framing.js`, `build-skill-bundles.js`, `lib/` (shared modules: `cascade.js`, `link-engine.js`, `parser.js`, `consistency.js`, etc.).
